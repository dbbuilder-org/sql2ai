...
BEGIN
    ...
    -- Process everything outside of dynamic SQL to avoid variable scoping issues
    ...
    DECLARE @AllChanges AS dbo.ChangesTableType
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN                        -- Skip entries containing "Code extracted with extended properties header"
                ...
                FROM @TempChanges as t
                WHERE IsValid = 1
                AND NOT EXISTS (
                    SELECT 1 FROM @AllChanges as A
                    WHERE A.ChangeDescription = t.ChangeDescription
                    AND A.ChangeAuthor = t.ChangeAuthor
                )
            END
        END
    END
    ...
    END    
    -- Call the PrintCode_ExtendedProperties procedure to process extended properties
    ...
    EXEC [dbo].[PrintCode_ExtendedProperties]
        @Object_ID = @Object_ID,
        ...
        @MetaSection = @MetaSection OUTPUT,
        @CodeReviewSection = @CodeReviewSection OUTPUT,
        @VersionSection = @VersionSection OUTPUT,
        @ReleaseNotesSection = @ReleaseNotesSection OUTPUT,
        @OtherSection = @OtherSection OUTPUT,
    ...
    -- Now format the object type for human readability
    ...
    -- Call the header procedure to build the header comment
    DECLARE @HeaderComment NVARCHAR(MAX)
    
    EXEC [dbo].[PrintCode_Header]
        @FormattedObjectType = @FormattedObjectType,
        @ActualObjectName = @ActualObjectName,
        @SchemaName = @SchemaName,
        @database_name = @database_name,
        ...
        @ExtendedPropertiesSection = @ExtendedPropertiesSection,
        @ChangesPreviousSection = @ChangesPreviousSection,
        @ChangesHeaderSection = @ChangesHeaderSection,
        @HeaderComment = @HeaderComment OUTPUT
    ...
    BEGIN
        EXEC sys.sp_addextendedproperty 
            ...
            @level0type = N''SCHEMA'', 
            @level0name = N''' + @SchemaName + ''',             @level1type = N''' + 
                CASE 
